...
app.post('/upload_csv', uploadCsv.single('csvFile'), (req, res) => {
  ...
  'FloweringTime_TH','FloweringTime_EN', 'FruitingTime_TH', 'FruitingTime_EN', 'Location_TH', 'Location_EN'  ];
    

  const columnImport = Object.keys(jsonObject[0]);
  ...
  function areWordListsEqual(list1, list2) {
    ...
    // Check if every word in list1 is present in list2
    return list1.every(word => list2.includes(word));
  }
  
  if (areWordListsEqual(columnStandard.slice(1), columnImport.slice(1))) {
    try  {
      let Noimg = [];
      const noImg = 'haveno.jpg';
      Noimg.push(noImg);
